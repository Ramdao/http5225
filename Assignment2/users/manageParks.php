    ...
    <table>
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
                <td>
                    <?php 
                        $imagePath = '../' . ltrim($row['ImagePath'], '/'); 
                        if (file_exists($imagePath) && !empty($row['ImagePath'])): 
                    ?>
                        <img src="<?= htmlspecialchars($imagePath); ?>" class="thumbnail" alt="<?= htmlspecialchars($row['ParkName']); ?>" width="100">
                    <?php else: ?>
